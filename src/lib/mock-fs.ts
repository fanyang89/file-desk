...
function createFile(name: string, content: string, mimeType?: string): MockFile {
  const timestamp = nowIso()
  return {
    kind: 'file',
    name,
...
function seedMockData() {
  ...
  documents.children.set('README.md', createFile('README.md', `# File Desk (Mock)\n\nThis preview is running with a browser mock API.\n`))
  documents.children.set('todo.txt', createFile('todo.txt', `- review PR preview\n- verify upload flow\n- test tab sorting\n`))
  notes.children.set('meeting-2026-02-08.md', createFile('meeting-2026-02-08.md', `## Notes\n\n- Added path bar mock fallback\n- Verified in Vercel preview\n`))

  const projectDir = createDir('file-desk')
  projectDir.children.set('package.json', createFile('package.json', `{\n  "name": "file-desk",\n  "private": true\n}\n`, 'application/json'))
  projectDir.children.set('main.tsx', createFile('main.tsx', `import { createRoot } from 'react-dom/client'\n\nconsole.log('mock preview')\n`))
...
export function mockCreateFolder(path: string, name: string): { success: boolean } {
  const normalizedPath = normalizePath(path)
  const dir = getDir(normalizedPath)
  if (!dir) throw new Error(`Directory not found: /${normalizedPath}`)
...
export function mockRenameEntry(path: string, oldName: string, newName: string): { success: boolean } {
  const normalizedPath = normalizePath(path)
  const dir = getDir(normalizedPath)
  if (!dir) throw new Error(`Directory not found: /${normalizedPath}`)
...
export function mockDeleteEntry(path: string, name: string): { success: boolean } {
  const normalizedPath = normalizePath(path)
  const dir = getDir(normalizedPath)
  if (!dir) throw new Error(`Directory not found: /${normalizedPath}`)

  const targetName = assertValidName(name)
  if (!dir.children.has(targetName)) throw new Error(`"${targetName}" does not exist`)
